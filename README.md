...
> 从v2.2.0版本开始，GrapeCoffee 支持符合OpenAI API标准格式的接口！
...
GrapeCoffee 基于 Python 3.13 和 PySide6 开发，本身具备良好的跨平台支持。若不想通过安装包安装，可下载源码后使用 PyInstaller 自行打包为单文件版本或支持你系统环境的版本。
...
当第一次使用时，需要在本地环境中搭建Ollama或者使用已有的Ollama服务。

点击应用的`设置`，在`Ollama大模型设置` > `模型地址`输入框输入你的ollama服务地址，格式如下：

```txt
http://192.168.3.88:11434
```

点击`刷新模型列表`，如果ollama服务地址正确且可用，将在`模型名称`下拉框中看到ollama中所有安装的模型。
...
如果不想使用Ollama服务，也可以使用支持标准OpenAI API接口的任意第三方服务。

点击应用的`设置`，在`OpenAI API设置` > `模型地址`输入框输入你的服务地址和`API KEY`中输入你的服务密钥。

点击`刷新模型列表`，如果你的服务地址正确且可用，点击后将在`模型名称`下拉框中看到该服务中所有模型。

> 有些第三方服务可能每个模型需要开通对应的使用权限，如果没有开通，即使选择也会报错。
...
如未部署 Ollama或没有支持标准OpenAI API接口的任意第三方服务，也可使用百度翻译 API。

> 官网 https://api.fanyi.baidu.com 申请免费 API，将获取的 `API ID` 和 `SecretKey` 填入`百度翻译API设置`中即可使用。
>
> 非广告，之所以支持该API只是作为一种备选方案而已。
